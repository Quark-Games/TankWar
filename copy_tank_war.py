...
class Tank:
    ...
    def show_tank(self):
        ...
        #barrel
        end_y = tankbase_h + self.y + math.sin(math.radians(self.angle)) * self.length
        end_x = int(tankbase_w/2) + self.x + math.cos(math.radians(self.angle)) * self.length
        pygame.draw.line(gameDisplay, self.b_color, (self.x + int(tankbase_w / 2), self.y + tankbase_h), (end_x, end_y), 5)
...
def game_loop():
    ...
    while True:
        ...
        if tank_x < 0:
            tank_x = 0
        if tank_x > display_width - tankbase_w:
            tank_x = display_width - tankbase_w
